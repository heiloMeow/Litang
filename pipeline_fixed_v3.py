...
def process_audio_offline(file_path: str, num_speakers: int):
    """
    API调用版本的离线处理函数
    返回结构化的结果，而不是直接保存文件
    """
    try:
        ...
        out_dir = make_session_dir(prefix=f"api_{base_name}")
        ...
        for seg in segments:
            ...
            try:
                ...
                # 失败的片段用噪声向量
                embeddings.append(np.random.normal(0, 1e-6, size=(192,)))
        ...
        # 保存到文件（可选）
        save_transcript_txt_json([
            Utterance(u["speaker"], u["start"], u["end"], u["text"]) 
            for u in utterances
        ], out_dir, "transcript")
